...
const { generateInteraktBody } = require("../../utils/interaktMsg");
      ...
      case "INTERAKT":
        ...
        try {
          ...
          const user = await strapi
            ...
            .findOne({
              where: {
                $or: [{ phone: phone }, { phone: phone.slice(-10) },{ phone: "+91" + phone.slice(-10) },],
              },
            });
    ...
    try {
      ...
      switch (global.user_verification_method) {
        ...
        case "INTERAKT":
          ...
          let reqData = generateInteraktBody(data);
          try {
            const resp = await axios.post(process.env.INTERAKT_URL, reqData, {
              headers: {
            ...
            return ctx.send({ message: "OTP Sent" }, 200);
          } catch (err) {
            console.log(err);
            return ctx.send(err.message, 500);
          }
          break;
